...
double applyF(double a, string op){
    ...
    if(op == "sin") return sin((a/180)*PI);
    if(op == "cos") return cos((a/180)*PI);
    if(op == "tan") return tan((a/180)*PI);
    if(op == "cotan") return 1/tan((a/180)*PI);
    if(op == "arcsin") return asin(a) * 180.0 / PI;
